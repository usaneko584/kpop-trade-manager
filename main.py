...
async def create_trade(
    request: Request, partner_name: str = Form(...), give_artist: str = Form(None), 
    give_item: str = Form(None), get_artist: str = Form(None), get_item: str = Form(None), 
    status: str = Form(...), memo: str = Form(None), is_public: str = Form("false"), 
    db: AsyncSession = Depends(get_db)
):
    user = get_user(request)
    if not user: return RedirectResponse(url="/", status_code=303)
    new_trade = Trade(
        user_id=user["user_id"], partner_name=partner_name, give_artist=give_artist,
        give_item=give_item, get_artist=get_artist, get_item=get_item,
        status=status, memo=memo, is_public=(is_public == "true")
    )
...
async def update_trade(
    ...
    give_artist: str = Form(None), give_item: str = Form(None), get_artist: str = Form(None), 
    get_item: str = Form(None), tracking_number: str = Form(None), memo: str = Form(None),
    is_public: str = Form("false"), db: AsyncSession = Depends(get_db)
):
    user = get_user(request)
    result = await db.execute(select(Trade).where(Trade.id == trade_id, Trade.user_id == user["user_id"]))
    trade = result.scalars().first()
    if trade:
        trade.partner_name = partner_name; trade.status = status; trade.give_artist = give_artist
        trade.give_item = give_item; trade.get_artist = get_artist; trade.get_item = get_item
        trade.tracking_number = tracking_number; trade.memo = memo; trade.is_public = (is_public == "true")
        await db.commit()
    return RedirectResponse(url="/", status_code=303)
